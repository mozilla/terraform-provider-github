...
- `members_can_delete_repositories` - Whether members with admin permissions can delete repositories.
- `members_can_change_repo_visibility` - Whether members with admin permissions can change repository visibility.
- `members_can_invite_outside_collaborators` - Whether members with admin permissions can invite outside collaborators.
- `members_can_delete_issues` - Whether members with admin permissions can delete issues.
- `display_commenter_full_name_setting_enabled` - Whether members can see commenters' full names in private repositories.
- `readers_can_create_discussions` - Whether users with read access can create and comment on discussions.
- `members_can_create_teams` - Whether organization members can create teams.
- `members_can_view_dependency_insights` - Whether organization members can view dependency insights.
- `default_repository_branch` - The default branch name for new repositories in the organization.
...
- `secret_scanning_validity_checks_enabled` - Whether secret scanning automatic validity checks are enabled for the organization.
...
- `secret_scanning_push_protection_custom_link` - The URL displayed to contributors who are blocked from pushing a secret by push protection.
